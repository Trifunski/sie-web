    ...
    <div class="flex flex-col max-w-screen-xl px-8 mx-auto md:items-center md:justify-between md:flex-row">
        <div class="flex flex-row items-center justify-between py-6">
            <div class="relative md:mt-2">
                <a href="/" class="text-lg ml-3 relative z-50 font-bold tracking-widest text-gray-900 rounded-lg focus:outline-none focus:shadow-outline">{{ $title }}</a>
        ...
        <nav :class="{ 'transform md:transform-none': !open, 'h-full': open }" class="h-0 md:h-auto flex flex-col flex-grow md:items-center pb-4 md:pb-0 md:flex md:justify-end md:flex-row origin-top duration-300 scale-y-0">
            @foreach($links as $link)
                @if ($link['name'] == 'Contact')
                    <a class="px-4 py-2 mt-2 text-sm bg-transparent text-white rounded-3xl md:mt-2 md:ml-4 hover:scale-110 duration-300 ease-in-out focus:outline-none focus:shadow-outline bg-yellow-500" href="{{ $link['url'] }}">{{ $link['name'] }}</a>
                @else
                    <a class="px-4 py-2 mt-2 text-sm bg-transparen rounded-lg md:mt-2 md:ml-4 hover:text-gray-900 focus:outline-none focus:shadow-outline" href="{{ $link['url'] }}">{{ $link['name'] }}</a>
                @endif
            @endforeach
            <!-- <a class="px-10 py-3 mt-2 text-sm text-center bg-white text-gray-800 rounded-full md:mt-2 md:ml-4" href="#">Login</a>
            <a class="px-10 py-3 mt-2 text-sm text-center bg-yellow-500 text-white rounded-full md:mt-2 md:ml-4" href="#">Sign Up</a> -->
        </nav>
    </div>
</div>